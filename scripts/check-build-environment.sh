...
[ "$#" -eq 0 ] || { echo 'usage: check-build-environment.sh' >&2; exit 78; }
...
if [ -z "$go_expected" ] || [ "$go_actual" != "$go_expected" ] || \
  ...
  printf 'TOOLCHAIN_MISMATCH: expectedGo=%s requiredRuntime=%s/%s actualGo=%s actualRuntime=%s/%s\n' \
    "${go_expected:-missing}" "${required_os:-unknown}" "${required_arch:-unknown}" \
